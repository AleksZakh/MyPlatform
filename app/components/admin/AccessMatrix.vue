...
const { snapshot, loading, saving, error, notice, needsReload, search, onlyAssigned,
  group, groups, rows, labels, load, toggle } = useAccessMatrix(props.kind, props.subjectId)
...
function disabled(row: AccessRow, action: AccessActionName): boolean {
  const cell = row.actions[action]
  return loading.value || saving.value || needsReload.value || (cell.directGranted ? !cell.canRevoke : !cell.canGrant)
}

function changePermission(row: AccessRow, action: AccessActionName, event: Event) {
  const input = event.target as HTMLInputElement
  const granted = input.checked
  input.checked = row.actions[action].directGranted
...
<template>
  <section class="access-matrix" aria-label="Назначение прав" :aria-busy="loading || saving">
    <header class="matrix-heading">
      <div>
        <h3>{{ kind === 'user' ? 'Личные и итоговые права' : kind === 'domainGroup' ? 'Права доменной группы' : 'Права подразделения' }}</h3>
    ...
    <template v-if="snapshot && !loading">
      ...
      <p v-else class="matrix-hint">
        {{ kind === 'user' ? 'Галочка меняет личное назначение. Ниже показан итоговый доступ и его источник.' : kind === 'domainGroup' ? 'Права наследуют участники группы безопасности AD, включая вложенные группы.' : `Назначения действуют для всех участников отдела (${snapshot.subject.memberCount}), включая будущих.` }}
      ...
      <div class="matrix-table-wrap" tabindex="0" role="region" aria-label="Список прав пользователя или группы">
        <table>
          ...
          <thead><tr><th scope="col">Ресурс</th><th v-for="action in snapshot.actions" :key="action" scope="col">{{ labels[action] }}</th></tr></thead>
          <tbody>
            <tr v-for="row in rows" :key="row.id">
              ...
              <td v-for="action in snapshot.actions" :key="action">
                <label class="grant-control">
                  <input type="checkbox" :checked="row.actions[action].directGranted" :disabled="disabled(row, action)"
                    :aria-label="`${row.name}: ${labels[action]}, ${kind === 'user' ? 'личное назначение' : kind === 'domainGroup' ? 'назначение группе AD' : 'назначение отделу'}`"
                    @change="changePermission(row, action, $event)" />
                  <span>{{ kind === 'user' ? 'Лично' : kind === 'domainGroup' ? 'Группе AD' : 'Отделу' }}</span>
                </label>
            ...
            <tr v-if="!rows.length"><td colspan="5">Ресурсы по этому фильтру не найдены.</td></tr>
          </tbody>
        </table>
      </div>
    </template>
