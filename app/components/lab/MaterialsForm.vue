...
async function loadManufacturers() {
  try {
    const response = await $fetch('/api/lab/manufacturer/all', {
      params: {
        sortKey: 'name',
        sortOrder: 'asc',
      },
...
// Загрузка материалов с сервера с пагинацией
async function loadMaterials() {
  isLoading.value = true;
  try {
    const response = await $fetch('/api/lab/material', {
      params: {
...
const saveMaterial = async () => {
  if (!currentMaterial.name?.trim()) {
    showTost(
      'Предупреждение!',
      'Название материала обязательно для заполнения',
      'warning',
      'fxemoji:warningsign',
      5000
    );
  ...
  try {
    const url = currentMaterial.id 
      ? `/api/lab/material/${currentMaterial.id}` 
      : '/api/lab/material';
    ...
    const response = await $fetch(url, {
      ...
      body: {
        ...
        manufacturerId: currentMaterial.manufacturerId || null,
        note: currentMaterial.note || null,
      },
    });
    ...
    if (response?.success) {
      showTost(
        'Успех!',
        currentMaterial.id ? 'Материал обновлен' : 'Материал добавлен',
        'success',
...
const deleteMaterial = async (id) => {
  ...
  if (material?._count?.receipts > 0) {
    showTost(
      'Невозможно удалить!',
      `Материал используется в ${material._count.receipts} поступлениях`,
      'warning',
      'fxemoji:warningsign',
      5000
    );
