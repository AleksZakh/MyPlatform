...
<script lang="ts" setup>
    ...
    const authUser = async () => {
        ...
        const loginValue = userLogin.value.split('\\')[1] ?? ''; // Получаем часть до '@' для поиска в AD
        ...
        console.log(`Пользователь ${user.user?.sAMAccountName || loginValue} ${exists ? 'найден' : 'не найден'}`, user);
        if(exists && Object.keys(user).length) {
            ...
            if(result.success) {
                ...
                setTimeout(async () => {
                    ...
                    showToast(`Пользователь ${result.data.user.name} - авторизован`, "success");
                }, 300);
                // await refreshSession();
