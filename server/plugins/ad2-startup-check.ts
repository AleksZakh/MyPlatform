    ...
    ad.authenticate(adConfig.username, adConfig.password, (err, isAuthenticated) => {
      if (err) {
        ...
        if (error.code === 'ENOTFOUND') {
          ...
          console.error('💡 Подключение отклонено. Проверьте порт и firewall');
        } else if (error.code === 'LDAP_INVALID_CREDENTIALS') {
          console.error('💡 Неверные учетные данные. Проверьте AD_USERNAME и AD_PASSWORD');
        } else if (error.code) {
          console.error(`💡 Код ошибки: ${error.code}`);
        }
        
        console.error('💡 Функции аутентификации и авторизации будут недоступны\n');
