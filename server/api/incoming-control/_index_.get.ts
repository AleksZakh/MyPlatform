...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (query.objectName) {
      where.objectName = {
        contains: query.objectName as string,
        mode: 'insensitive',
      };
    }

    if (query.samplingActNumber) {
      where.samplingActNumber = {
        contains: query.samplingActNumber as string,
        mode: 'insensitive',
      };
    }
    ...
    if (query.samplingPlace) {
      where.samplingPlace = {
        contains: query.samplingPlace as string,
        mode: 'insensitive',
      };
    }

    if (query.personProvidedSample) {
      where.personProvidedSample = {
        contains: query.personProvidedSample as string,
        mode: 'insensitive',
      };
    }
    ...
    if (query.materialName) {
      where.materialName = {
        contains: query.materialName as string,
        mode: 'insensitive',
      };
    }

    if (query.qualityDocument) {
      where.qualityDocument = {
        contains: query.qualityDocument as string,
        mode: 'insensitive',
      };
    }
    ...
    if (query.manufacturer) {
      where.manufacturer = {
        contains: query.manufacturer as string,
        mode: 'insensitive',
      };
    }
    ...
    if (query.protocolNumber) {
      where.protocolNumber = {
        contains: query.protocolNumber as string,
        mode: 'insensitive',
      };
    }
    ...
    const transformedData = data.map((item) => ({
      ...
      'Дата отбора проб': item.samplingDate
        ? new Date(item.samplingDate).toLocaleDateString('ru-RU')
        : '',
      ...
      'Дата поступления материала': item.materialReceiptDate
        ? new Date(item.materialReceiptDate).toLocaleDateString('ru-RU')
        : '',
      ...
      'Дата протокола': item.protocolDate
        ? new Date(item.protocolDate).toLocaleDateString('ru-RU')
        : '',
